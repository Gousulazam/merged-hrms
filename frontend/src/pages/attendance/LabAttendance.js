...
export default function LabAttendance(props) {
    ...
    const [lessonPlan, setLessonPlan] = useState([{ topic: '',status: '' }]);
    ...
    const coAdd = (e) => {
        e.preventDefault();
        navigate("/addlabattendance", { state: { academicYear, subject, semType, date, startTime, endTime ,lessonPlan } });
    };
    ...
    useEffect(() => {
        axios.post(`${props.baseURL}/getlablessonplantopic`, {
            id: subject
        })
            .then((response) => {
    ...
    return (
        <div className="card">
            <div className="card-header text-center font-weight-bold">Lab Attendance</div>
            <form onSubmit={coAdd}>
