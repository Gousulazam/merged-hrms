...
const ApproveAdmission = (props) => {
    ...
    const approve = (e) => {
        e.preventDefault();
        axios.post(`${props.baseURL}/updateStudentStatus`, {
            id: e.target.value,
            cid: props.userDetails.cid,
            role: state.role,
            value: "approved"
    ...
    return (
        <React.Fragment>
        {redirect ? <Navigate to="/Admissionformapproval" /> : null}
        <div className="card">
            ...
            <div className=" card-footer text-center">
                <button type="button" onClick={approve} className="btn btn-success rounded mx-2" value={state.id} >Approve</button>
                <button type="button" onClick={reject} className="btn btn-danger rounded" value={state.id} >Reject</button>
            </div>
        </div>
        </React.Fragment>
    )
